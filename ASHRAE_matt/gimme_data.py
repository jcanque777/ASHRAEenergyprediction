...
def all_the_data(train, building, weather_data, dummies_=True, time__index=False):
    '''
    train: initial data set(either train or test)
    building: building dataset
    weather_data: weather dataset (either weather_train or weather_test)
    
    dummies: True, returns 4 dummy columns for type of meter
             False, returns 4 columns for meter_values, with corresponding meter readings in each col.
    
    time_index: returns a dataset with an index of timestamps
    '''
...
#     Holiday and Date Time Conversions
    ...
    return train_meter_columns(df_converted_dates, weather_data, time_index=time__index, dummies=dummies_)
